...
const services = [
  { number: "01", title: "AI product engineering", tag: "BUILD", detail: "Production architecture for agentic applications, retrieval, tool use, model routing, durable workflows, and the integrations that make them useful.", outcome: "From impressive prototype to dependable product." },
  { number: "02", title: "AI security", tag: "ATTACK / DEFEND", detail: "Threat modeling, adversarial testing, red teaming, permissions, control design, and operational defenses for systems that reason and act.", outcome: "Know how the system breaks before an attacker does." },
  { number: "03", title: "Evals & reliability", tag: "PROVE", detail: "Curated datasets, scenario evaluations, deterministic and LLM judges, failure taxonomies, regression gates, and measurable improvement loops.", outcome: "Replace launch anxiety with decision-quality evidence." },
...
function sendBrief(event: FormEvent<HTMLFormElement>) {
  ...
  const lines = [
    ...
    data.get("focus") ? `Focus: ${data.get("focus")}` : "",
    "",
    String(data.get("context") || ""),
  ].filter(Boolean);
  window.location.href = `mailto:dan@danlevy.net?subject=${encodeURIComponent("17th Street Labs — project brief")}&body=${encodeURIComponent(lines.join("\n"))}`;
}
...
function ContactDialog({ children }: { children: ReactNode }) {
  return <Dialog><DialogTrigger asChild>{children}</DialogTrigger><DialogContent className="contact-dialog sm:max-w-2xl"><DialogHeader><span className="micro">NEW ENGAGEMENT</span><DialogTitle>What hard problem are you carrying?</DialogTitle><DialogDescription>Give us the useful version: what you are building, what is uncertain, and what happens if it fails.</DialogDescription></DialogHeader><form onSubmit={sendBrief}><div className="form-row"><label>Name<input name="name" required placeholder="Your name" /></label><label>Work email<input name="email" required type="email" placeholder="you@company.com" /></label></div><label>Focus<select name="focus" defaultValue=""><option value="" disabled>Select an area</option><option>AI product engineering</option><option>AI security</option><option>Evals & reliability</option><option>Technical advisory</option></select></label><label>Context<textarea name="context" required rows={5} placeholder="What are you building?" /></label><button type="submit" className="submit-button">Open project brief <ArrowUpRight size={17} /></button></form></DialogContent></Dialog>;
}
...
function HomeView({ navigate }: { navigate: (v: View) => void }) {
  return <>
    ...
    <section className="research-feature"><div className="research-visual" aria-hidden="true"><div className="scan-grid"/><span className="scan-line"/><div className="finding f1"><i/>prompt boundary</div><div className="finding f2"><i/>tool escalation</div><div className="finding f3"><i/>evidence captured</div></div><div className="research-copy"><div className="section-label"><span>03</span> FIELD WORK</div><span className="micro">OPEN-SOURCE SECURITY RESEARCH</span><h2>ExploitHunter</h2><p>A local-first offensive-security and agent-evaluation lab for studying how models discover, validate, and communicate findings across long-running attack workflows.</p><div className="research-stats"><div><b>412M+</b><span>evaluated tokens</span></div><div><b>173</b><span>model profiles</span></div><div><b>4,835</b><span>evaluation rows</span></div></div><ul><li>Deterministic and LLM judges</li><li>Trajectory, evidence, cost, and tool telemetry</li><li>Reproducible regression gates</li></ul><a href="https://exploithunter.app" target="_blank" rel="noreferrer">Explore the research <ArrowUpRight size={17}/></a></div></section>
...
function AboutView() {
  return <><section className="interior-hero about-hero"><div className="section-label"><span>01</span> THE LAB</div><h1>Experience without<br /><em>the institution.</em></h1><p>17th Street Labs is a founder-led AI engineering and security research consultancy. Small by design, direct by default, and built for work where judgment matters.</p></section><section className="numbers"><div><b>25+</b><span>years building and leading software systems</span></div><div><b>6→60</b><span>engineers scaled in a prior platform organization</span></div><div><b>250k/s</b><span>events delivered at production scale</span></div><div><b>50M</b><span>users reached by a platform Dan helped build</span></div></section><section className="principles-section"><div className="section-label"><span>02</span> OPERATING PRINCIPLES</div><div className="principle-stack"><div><span>01</span><h3>Evidence over theater.</h3><p>A fluent demo is not a reliable system. We design measurement that can survive scrutiny.</p></div><div><span>02</span><h3>Security is architecture.</h3><p>Permissions, boundaries, data flows, and failure recovery belong in the design, not the launch checklist.</p></div><div><span>03</span><h3>Capability must transfer.</h3><p>The engagement compounds when your team leaves with sharper systems and stronger judgment.</p></div></div></section><section className="founders"><div className="section-label"><span>03</span> FOUNDERS</div><div className="founder-grid"><SpotlightCard><div className="founder-monogram marina">ML</div><span className="micro">CO-FOUNDER · CHIEF EXECUTIVE OFFICER</span><h2>Marina Levy</h2><p>Marina sets the operating direction for 17th Street Labs, shaping ambitious technical work into focused engagements, durable partnerships, and measurable client value.</p></SpotlightCard><SpotlightCard><div className="founder-monogram">DL</div><span className="micro">CO-FOUNDER · PRINCIPAL AI ENGINEER</span><h2>Dan Levy</h2><p>Engineer, educator, founder, and technical leader with 25+ years across production systems, developer platforms, AI applications, evaluation infrastructure, and offensive and defensive AI security.</p><a href="mailto:dan@danlevy.net">dan@danlevy.net <ArrowUpRight size={15}/></a></SpotlightCard></div></section></>;
}

function ContactView() {
  return <section className="contact-view"><div><div className="section-label"><span>01</span> CONTACT</div><h1>Bring the<br /><em>hard problem.</em></h1><p>Tell us what you are building, where confidence breaks down, and what a meaningful result would change.</p><div className="contact-people"><span>ENGAGEMENTS</span><a href="mailto:dan@danlevy.net">dan@danlevy.net</a><span>LED BY</span><strong>Marina Levy · CEO<br/>Dan Levy · Principal AI Engineer</strong></div></div><SpotlightCard className="contact-panel"><span className="micro">START HERE</span><h2>Three useful sentences are enough.</h2><form onSubmit={sendBrief}><label>Name<input name="name" required placeholder="Your name" /></label><label>Work email<input name="email" required type="email" placeholder="you@company.com" /></label><label>What is at stake?<textarea name="context" required rows={6} placeholder="The system, the uncertainty, and the consequence." /></label><button className="submit-button" type="submit">Open project brief <ArrowUpRight size={17}/></button></form></SpotlightCard></section>;
}

function Footer({ navigate }: { navigate: (v: View) => void }) {
  return <footer><div><button className="brand" onClick={() => navigate("home")}><Mark/><span>17th Street Labs</span></button><p>Engineering intelligence.<br/>Securing agency.</p></div><div className="footer-nav">{(["home", "services", "about", "contact"] as View[]).map(item => <button key={item} onClick={() => navigate(item)}>{item}</button>)}</div><div className="footer-meta"><span>AI SYSTEMS · EVALS · SECURITY</span><span>DENVER · WORKING GLOBALLY</span><span>© 2026 17TH STREET LABS LLC</span></div></footer>;
}
