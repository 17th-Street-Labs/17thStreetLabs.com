...
export interface WebThreadsOptions {
  color1?: string;
  color2?: string;
  color3?: string;
  speed?: number;
...
/**
 * Mounts the animated thread field into `container`.
 * Returns a teardown function that releases the GL context.
 */
export function initWebThreads(
  container: HTMLElement,
  options: WebThreadsOptions = {},
): () => void {
  const {
    color1 = "#5227FF",
    color2 = "#FF9FFC",
  ...
  const program = new Program(gl, {
    ...
    uniforms: {
      ...
      uRainbowSpeed: { value: rainbowSpeed },
      uRainbowSpread: { value: rainbowSpread },
      uBackgroundColor: { value: new Float32Array(rgbBg) },
  ...
  const uTime = program.uniforms.iTime as { value: number };
  const uRes = program.uniforms.iResolution as { value: Float32Array };
  const uMouse = program.uniforms.uMouse as { value: Float32Array };
  const uMouseActive = program.uniforms.uMouseActive as { value: number };
  ...
  const setSize = () => {
    ...
    uRes.value[0] = gl.drawingBufferWidth;
    uRes.value[1] = gl.drawingBufferHeight;
  ...
  const onPointerVelocity = (e: PointerEvent) => {
    ...
    if (lastPointer) {
      ...
      boost = Math.min(boost + (dist / dt) * 0.35, 6);
    }
  ...
  const loop = (t: number) => {
    ...
    clockSpeed += (1 + boost - clockSpeed) * (1 - Math.exp(-dt / 250));
    clock += dt * 0.001 * clockSpeed;
    uTime.value = clock;
    currentMouse[0] += 0.05 * (targetMouse[0] - currentMouse[0]);
    currentMouse[1] += 0.05 * (targetMouse[1] - currentMouse[1]);
    currentActive += 0.05 * (targetActive - currentActive);
    uMouse.value[0] = currentMouse[0];
    uMouse.value[1] = currentMouse[1];
    uMouseActive.value = currentActive;
    renderer.render({ scene: mesh });
